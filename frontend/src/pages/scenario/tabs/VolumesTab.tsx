...
import { ScenarioDetail, ProductMonth } from "../../../types/scenario";
...
import { buildMonthsList, isInWindow, monthIndex } from "../../../utils/scenarioWindow";

type VolumesMap = Record<number, number[]>; // productId -> quantities

export default function VolumesTab({ data, refresh }: { data: ScenarioDetail; refresh: () => void }) {
  const monthsList = buildMonthsList(data);
  ...
  const extrapolateAll = () => {
    ...
    setVolumes((v) => {
      ...
      for (const p of data.products) {
        ...
        for (let i = 1; i < months; i++) row[i] = exMode === "constant" ? first : Math.round(first * Math.pow(1 + rate, i));
        copy[p.id] = row;
      }
      return copy;
    });
  ...
  const saveAllVolumes = async () => {
    try {
      for (const p of data.products) {
        ...
        const payload: ProductMonth[] = arr.map((q, i) => ({ year: monthsList[i].y, month: monthsList[i].m, quantity: Number(q || 0) }));
    ...
    <div className="space-y-4">
      <Card>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
          ...
          <div className="flex md:justify-end">
            <button onClick={extrapolateAll} className="px-3 py-2 rounded-md border text-sm hover:bg-gray-50">Extrapolate All</button>
          </div>
        </div>
      </Card>
      ...
      <Card>
          ...
          <table className="min-w-max text-xs table-fixed">
            <thead>
              <tr className="border-b">
                <th className="sticky left-0 z-10 bg-white py-1 px-2 text-left" style={{ width: FIRST_COL_W, minWidth: FIRST_COL_W }}>Product</th>
                {monthsList.map((mm, i) => (
                  <th key={i} className="py-1 px-2 text-right whitespace-nowrap" style={{ width: MONTH_COL_W, minWidth: MONTH_COL_W }}>{fmtMonthYY(mm.y, mm.m)}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {data.products.map((p) => (
                <tr key={p.id} className="border-b last:border-0">
                  <td className="sticky left-0 z-10 bg-white py-1 px-2 font-medium" style={{ width: FIRST_COL_W, minWidth: FIRST_COL_W }}>{p.name}</td>
                  {(volumes[p.id] ?? new Array(data.months).fill(0)).map((q, i) => (
                    <td key={i} className="py-1 px-2 text-right align-middle" style={{ width: MONTH_COL_W, minWidth: MONTH_COL_W }}>
                      <input type="number" value={q ?? 0} onChange={(e) => setCell(p.id, i, Number(e.target.value))} className="w-full px-1.5 py-1 rounded border text-xs text-right tabular-nums" />
                    </td>
                  ))}
                </tr>
              ))}
        ...
        <div className="mt-3 flex justify-end">
          <button onClick={saveAllVolumes} className="px-3 py-1.5 rounded-md bg-indigo-600 text-white text-sm hover:bg-indigo-500">Save All</button>
        </div>
      </Card>
