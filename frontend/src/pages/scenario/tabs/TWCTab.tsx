...
import { ScenarioDetail, CapexRow, FinanceMode, PLResponse } from "../../../types/scenario";
import { Card, Field, KV } from "../../../components/ui";
import { fmt, fmtMonthYY, getNumberClass, FIRST_COL_W, MONTH_COL_W } from "../../../utils/format";
import { isInWindow, monthIndex } from "../../../utils/scenarioWindow";

export default function TWCTab({ data, pl }: { data: ScenarioDetail; pl: PLResponse | null }) {
  ...
  const addCapexRow = () => setCapexRows((r) => [...r, { year: new Date(data.start_date).getUTCFullYear(), month: new Date(data.start_date).getUTCMonth()+1, amount: 0 }]);
  const changeCapexRow = (i: number, next: CapexRow) => setCapexRows((rows) => rows.map((r, idx) => (idx === i ? next : r)));
  const removeCapexRow = (i: number) => setCapexRows((rows) => rows.filter((_, idx) => idx !== i));
  ...
  const finance = useMemo(() => {
    ...
    if (treatFirstAsT0) {
      ...
      for (const row of capexRows) {
        ...
        const sy = d.getUTCFullYear(); const sm = d.getUTCMonth() + 1;
        if (row.year === sy && row.month === sm) firstMonthCapex += Number(row.amount || 0);
      }
    ...
    const ds = Number(dso || "0"), dp = Number(dpo || "0"), di = Number(dio || "0");
    ...
    const npv = (cf: number[], rate: number) => cf.reduce((acc, v, i) => acc + v / Math.pow(1 + rate, i + 1), 0);
    ...
    const preview = ["t0:" + fmt(t0), ...selectedCF.slice(0, 8).map((v, i) => `t${i + 1}:${fmt(v)}`)].join(" | ");

    return { rMonthly: r, havePL, npv: projectNPV, irrMonthly, irrAnnual, preview, cfBasis: mode === "proxy" ? "Net Income (proxy)" : "Free Cash Flow", labels, ebit, taxes, depreciation, capex, deltaWC, fcf };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      ...
      <Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <Field label="WACC % (annual)"><input type="number" value={wacc} onChange={(e) => setWacc(e.target.value)} className="w-full px-3 py-2 rounded-md border text-sm" /></Field>
          <Field label="Tax Rate % (annual)"><input type="number" value={taxRate} onChange={(e) => setTaxRate(e.target.value)} className="w-full px-3 py-2 rounded-md border text-sm" /></Field>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3">
          <Field label="DSO (days)"><input type="number" value={dso} onChange={(e) => setDso(e.target.value)} className="w-full px-3 py-2 rounded-md border text-sm" /></Field>
          <Field label="DPO (days)"><input type="number" value={dpo} onChange={(e) => setDpo(e.target.value)} className="w-full px-3 py-2 rounded-md border text-sm" /></Field>
          <Field label="DIO (days)"><input type="number" value={dio} onChange={(e) => setDio(e.target.value)} className="w-full px-3 py-2 rounded-md border text-sm" /></Field>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
          <Field label="Depreciation Life (years)">
            <input type="number" value={deprLifeYears} min={1} step={1} onChange={(e) => setDeprLifeYears(e.target.value)} className="w-full px-3 py-2 rounded-md border text-sm" />
          </Field>
      ...
      <Card>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead><tr className="text-left border-b"><th className="py-1 pr-3">Year</th><th className="py-1 pr-3">Month</th><th className="py-1 pr-3">Amount</th><th className="py-1 pr-3 w-16"></th></tr></thead>
            <tbody>
              {capexRows.length === 0 ? <tr><td colSpan={4} className="py-2 text-gray-500">No capex yet.</td></tr> :
                capexRows.map((r, idx) => {
                  const out = !isInWindow(data, r.year, r.month);
                  const danger = out ? "border-red-400 focus:ring-red-500" : "";
                  return (
                    <tr key={idx} className="border-b last:border-0">
                      <td className="py-1 pr-3"><input type="number" value={r.year} onChange={(e) => changeCapexRow(idx, { ...r, year: Number(e.target.value) })} className={`w-24 px-2 py-1 rounded border text-sm ${danger}`} /></td>
                      <td className="py-1 pr-3"><input type="number" value={r.month} min={1} max={12} onChange={(e) => changeCapexRow(idx, { ...r, month: Number(e.target.value) })} className={`w-20 px-2 py-1 rounded border text-sm ${danger}`} /></td>
                      <td className="py-1 pr-3"><input type="number" value={r.amount} onChange={(e) => changeCapexRow(idx, { ...r, amount: Number(e.target.value) })} className="w-32 px-2 py-1 rounded border text-sm" /></td>
                      <td className="py-1 pr-3 text-right"><button onClick={() => removeCapexRow(idx)} className="px-2 py-1 rounded border hover:bg-gray-50">Remove</button></td>
                    </tr>
                  );
                })}
            </tbody>
        ...
        {!finance?.havePL ? (
          ...
          <>
            ...
            <div className="text-xs text-gray-500 mt-3">
              Preview (first 8): <code className="text-[11px]">{finance.preview}</code>
            </div>
          </>
        )}
      </Card>
              ...
              <table className="min-w-max text-xs table-fixed">
                <thead>
                  <tr className="border-b">
                    <th className="sticky left-0 bg-white py-1 px-2 text-left" style={{ width: FIRST_COL_W, minWidth: FIRST_COL_W }}>Line</th>
                    {finance.labels.map((lm, i) => (
                      <th key={i} className="py-1 px-2 text-right whitespace-nowrap" style={{ width: MONTH_COL_W, minWidth: MONTH_COL_W }}>
                ...
                <tbody>
                  {[
                    ...
                    <tr key={row.label} className="border-b last:border-0">
                      <td className="sticky left-0 bg-white py-1 px-2 font-medium" style={{ width: FIRST_COL_W, minWidth: FIRST_COL_W }}>{row.label}</td>
                      {row.arr.map((v: number, idx: number) => (
                        <td key={idx} className={`py-1 px-2 text-right ${getNumberClass(v)}`} style={{ width: MONTH_COL_W, minWidth: MONTH_COL_W }}>{fmt(v)}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
